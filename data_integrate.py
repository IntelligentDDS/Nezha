    ...
    def show_graph(self):
        for key in self.adjacency_list.keys():
            logger.info("head:%s %s" %
                        (key.event, from_id_to_template(key.event)))
            for item in self.adjacency_list[key]:
                logger.info("tail:%s %s" %
                            (item.event, from_id_to_template(item.event)))
            logger.info("----")
                ...
                for item in self.adjacency_list[key]:
                    if traget_event == item.event:
                        ...
                        if deepth == 0:
                            pod = item.pod
                        flag = True
                        if "start" in from_id_to_template(key.event) and "TraceID" not in from_id_to_template(key.event):
                            deepth = deepth + 1
                        break
                if flag == True:
                    break
...
def get_events_within_trace(trace_reader, log_reader, trace_id, alarm_list):
    """
        ...
        if len(spans['SpanID']) > 0:
            ...
            for span_index in range(len(spans['SpanID'])):
                ...
                span.append_event(Event(timestamp=np.ceil(spans['StartTimeUnixNano'].iloc[span_index]).astype(int),
                                        event=log_parsing(log=start_event, pod=pod), pod=pod, spanid=span_id, parentid=parent_id))
                        ...
                        if len(logs['TimeUnixNano']) > 0:
                            for log_index in range(len(logs['TimeUnixNano'])):
                                ...
                                span.append_event(Event(timestamp=timestamp,
                                                        event=log_parsing(log=log, pod=pod), pod=pod, spanid=span_id, parentid=parent_id))
                ...
                span.append_event(Event(timestamp=end_timestamp,
                                        event=log_parsing(log=end_event, pod=pod), pod=pod, spanid=span_id, parentid=parent_id))
                # alarm event
                # hipster
                        ...
                        if alarm_dict["pod"] == span.pod:
                            ...
                            for index in range(len(alarm_dict["alarm"])):
                                span.append_event(Event(timestamp=np.ceil(spans['StartTimeUnixNano'].iloc[span_index]).astype(int) + index + 1,
                                                        event=log_parsing(log=alarm_dict["alarm"][index]["metric_type"], pod="alarm"), pod=pod, spanid=span_id, parentid=parent_id))
                ...
                # sort event by event timestamp
                span.sort_events()
                # if pod == "ts-verification-code-service-7b6dc75c45-r66rb":
                #     span.show_all_events()
...
def generate_event_chain(alarm_list, trace):
    """
            ...
            for index in range(len(pod_event_chain_list)):
                if re.search(pod, pod_event_chain_list[index]):
                    for alarm in pod_alarm['alarm']:
                        event = log_parsing(
                            log=alarm['metric_type'], pod="alarm")
...
def data_integrate(trace_file, trace_id_file, log_file, alarm_list):
    """
    ...
    trace_id_reader = pd.read_csv(
        trace_id_file, index_col=False, header=None)
    trace_reader = pd.read_csv(
        trace_file, index_col='TraceID', usecols=['TraceID', 'SpanID', 'ParentID', 'PodName', 'StartTimeUnixNano', 'EndTimeUnixNano', 'OperationName'])
    log_reader = pd.read_csv(log_file, index_col='SpanID', usecols=[
        'TimeUnixNano', 'SpanID', 'Log'])
    log_sequences = []
    traces = []
...
if __name__ == '__main__':
    date = "2023-01-29"
    hour_min = "08_50"
    construction_data_path = "/root/jupyter/nezha/construction_data"
